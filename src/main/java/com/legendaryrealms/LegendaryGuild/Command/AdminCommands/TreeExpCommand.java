...
public class TreeExpCommand extends LegendaryCommand {
    public TreeExpCommand() {
        super("legendaryguild.admin", "treeexp", 5, true);
    }
    ...
    public void handle(CommandSender sender, String[] args) {
        ...
        switch (aot.toLowerCase()){
            case "add" : {
                GuildAPI.addGuildTreeExp(sender.getName(),guild,amount);
                sender.sendMessage(lang.plugin+lang.admin_treeexp_add.replace("%target%",guildName).replace("%value%",amountStr));
                break;
            }
            case "take" : {
                GuildAPI.takeGuildTreeExp(sender.getName(),guild,amount);
                sender.sendMessage(lang.plugin+lang.admin_treeexp_take.replace("%target%",guildName).replace("%value%",amountStr));
                break;
            }
            case "set" : {
                GuildAPI.setGuildTreeExp(sender.getName(),guild,amount);
                sender.sendMessage(lang.plugin+lang.admin_treeexp_set.replace("%target%",guildName).replace("%value%",amountStr));
                break;
            }
        }
    }
    ...
    public List<String> complete(CommandSender sender, String[] args) {
        return new CommandTabBuilder()
                .addTab(Arrays.asList("add","set","take"),2,Arrays.asList("treeexp"),1)
