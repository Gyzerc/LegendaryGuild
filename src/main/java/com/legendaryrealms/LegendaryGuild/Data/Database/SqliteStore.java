...
//        }
        try {
            Optional<ResultSet> optionalResultSet = getDataStringResult(connection,DatabaseTable.SYSTEM_PLACEHODER.getBuilder(), key);
...
//        }
        try {
            ...
            if (resultSetOptional.isPresent()) {
                ResultSet rs = resultSetOptional.get();
                return Guild_Redpacket.toData(guild,rs.getString("data"));
            }
        } catch (SQLException e) {
            legendaryGuild.info("获取公会红包数据时出错！ ",Level.SEVERE,e);
        }
...
//        }
        try {
            ...
            if (resultSetOptional.isPresent()) {
                ResultSet rs = resultSetOptional.get();
                return GuildStore.toStore(guild,rs.getString("data"));
            }
        } catch (SQLException e) {
            legendaryGuild.info("获取公会仓库数据时出错！ ",Level.SEVERE,e);
        }
