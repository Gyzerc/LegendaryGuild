...
public class GuildMembersRequirement extends Requirement {
    @Override
    ...
    public boolean canPass(Player p, String str) {
        ...
        if (guild.getMembers().size() >= Double.parseDouble(args[1])){
            return true;
        }
        p.sendMessage(lang.plugin+lang.noenough_members.replace("%value%",args[1]));
        return false;
    }
